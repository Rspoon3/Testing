    ...
    var body: some View {
        Canvas { context, canvasSize in
            ...
            for col in 0...cols {
                ...
                context.stroke(line, with: .color(accentColor.opacity(0.12)), lineWidth: 1)
            }
            ...
            let neonAccents: [Color] = [
                accentColor, .pink, .purple, .mint, Color(red: 0.4, green: 1.0, blue: 0.6)
            ]
            ...
            for _ in 0..<15 {
                ...
                let x = CGFloat(col) * spacing
                let y = CGFloat(row) * spacing
                let colorIndex = Int(rng.nextDouble() * Double(neonAccents.count)) % neonAccents.count
                ...
                context.fill(outerGlow, with: .color(neonAccents[colorIndex].opacity(opacity * 0.3)))
                ...
                context.fill(innerDot, with: .color(neonAccents[colorIndex].opacity(opacity)))
            }
            ...
            for _ in 0..<4 {
                ...
                context.stroke(line, with: .color(neonAccents[colorIndex].opacity(opacity)), lineWidth: 2)
            }
        }
    }
}
